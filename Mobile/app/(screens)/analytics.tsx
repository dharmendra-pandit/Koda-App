...
export default function AnalyticsScreen() {
  ...
  const totalSubjectMins =
    analytics?.bySubject.reduce((a, s) => a + s.totalMinutes, 0) || 1
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.summaryRow}>
            {[
              {
                label: 'Total Time',
                value: fmt(analytics?.totalMinutes ?? 0),
                icon: BarChart2,
                color: Colors.primary,
              },
              {
                    ...
                    >
                      ...
                      <View style={styles.barTrack}>
                        <MotiView
                          ...
                          animate={{
                            height:
                              `${Math.max(ratio * 100, item.mins > 0 ? 5 : 0)}%` as any,
                          }}
            ...
            <View style={styles.subjectList}>
              {(analytics?.bySubject.length ?? 0) === 0 ? (
                <Text style={styles.emptyText}>
                  No subject data yet. Start a study session!
                </Text>
              ) : (
                  ...
                  >
                    <Card style={styles.subjectItem}>
                      <View
                        style={[
                          styles.subjectDot,
                          { backgroundColor: s.subjectColor || Colors.primary },
                        ]}
                      />
                      <View style={{ flex: 1 }}>
                        <View style={styles.subjectRow}>
                          <Text style={styles.subjectName}>
                            {s.subjectTitle || 'General'}
                          </Text>
                          <Text style={styles.subjectTime}>
                            {fmt(s.totalMinutes)} (
                            {Math.round(
                              (s.totalMinutes / totalSubjectMins) * 100,
                            )}
                            %)
                          </Text>
                        </View>
