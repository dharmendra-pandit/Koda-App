...
import { ArrowLeft, ThumbsUp, CheckCircle, Send, Reply as ReplyIcon, X, HelpCircle } from 'lucide-react-native';
...
function ReplyCard({ 
  ...
  const handleUpvote = () => {
    scale.value = withSpring(1.2, { damping: 10 }, () => { scale.value = withSpring(1); });
    onUpvote();
  };

  const isUpvoted = useAuthStore.getState().user?._id && Array.isArray(reply.upvotes) && reply.upvotes.includes(useAuthStore.getState().user?._id);
  ...
  return (
    ...
    >
      ...
      <Card style={reply.isAccepted ? [styles.replyCard, styles.acceptedCard] : styles.replyCard}>
        {reply.isAccepted && (
        ...
        <View style={styles.replyHeader}>
          <Avatar uri={reply.userId?.avatar} name={reply.userId?.name} size={32} />
          <View style={{ flex: 1 }}>
        ...
        <View style={styles.replyActions}>
          <TouchableOpacity onPress={handleUpvote} style={styles.upvoteRow}>
            <Animated.View style={anim}>
              <ThumbsUp size={16} color={isUpvoted ? Colors.primary : Colors.textMuted} fill={isUpvoted ? Colors.primary : 'none'} />
            </Animated.View>
            <Text style={[styles.upvoteCount, isUpvoted && { color: Colors.primary }]}>{reply.upvotes?.length ?? 0}</Text>
          </TouchableOpacity>
...
export default function DoubtDetailScreen() {
  ...
  useEffect(() => {
    ...
    socketService.on('reply_updated', (updatedData) => {
      setReplies((prev: any[]) => prev.map(r => 
    ...
    socketService.on('doubt_updated', (updatedData) => {
      if (updatedData._id === id) {
        setDoubt((prev: any) => prev ? { ...prev, upvotes: Array(updatedData.upvotes).fill(0) } : prev);
      }
    });
    ...
    socketService.on('doubt_solved', (data) => {
      if (data._id === id) {
        ...
        setReplies((prev: any[]) => prev.map(r => ({ ...r, isAccepted: r.userId?._id === data.solvedBy?._id })));
      }
    });
    ...
    try {
      ...
      setDoubt({ 
        ...
        upvotes: data.data.upvoted 
          ...
          : (Array.isArray(doubt.upvotes) ? doubt.upvotes : []).filter((id: string) => id !== user?._id)
      });
    ...
    try {
      ...
      setReplies((prev) => prev.map((r) => r._id === replyId ? { 
        ...
        upvotes: data.data.upvoted
          ...
          : (Array.isArray(r.upvotes) ? r.upvotes : []).filter((id: string) => id !== user?._id)
      } : r));
    } catch {}
  };
  ...
  const handleAccept = async (replyId: string) => {
    try {
      ...
      setReplies((prev) => prev.map((r) => ({ ...r, isAccepted: r._id === replyId })));
  ...
  const isDoubtUpvoted = Array.isArray(doubt?.upvotes) && doubt.upvotes.includes(user?._id);
                ...
                <MotiView from={{ opacity: 0, translateY: -10 }} animate={{ opacity: 1, translateY: 0 }} transition={{ type: 'timing', duration: 400 }}>
                  <Card style={styles.doubtCard}>
                    <View style={styles.doubtHeaderRow}>
                      <Badge label={doubt.subjectId?.title || 'General'} color={doubt.subjectId?.color + '20' || Colors.primary + '20'} textColor={doubt.subjectId?.color || Colors.primary} />
                      {doubt.isSolved && (
                    ...
                    {doubt.description ? <Text style={styles.doubtDesc}>{doubt.description}</Text> : null}
                    
                    <View style={styles.doubtFooter}>
                      <View style={styles.doubtMeta}>
                        <Avatar uri={doubt.userId?.avatar} name={doubt.userId?.name} size={28} />
                        <View>
                          <Text style={styles.doubtUser}>{doubt.userId?.name}</Text>
                          <Text style={styles.doubtTime}>{timeAgo(doubt.createdAt)}</Text>
                        </View>
                      </View>
                      <TouchableOpacity onPress={handleUpvoteDoubt} style={[styles.doubtUpvote, isDoubtUpvoted && styles.doubtUpvoted]}>
                        <ThumbsUp size={18} color={isDoubtUpvoted ? Colors.white : Colors.primary} fill={isDoubtUpvoted ? Colors.white : 'none'} />
                        <Text style={[styles.doubtUpvoteText, isDoubtUpvoted && { color: Colors.white }]}>{doubt.upvotes?.length || 0}</Text>
                      </TouchableOpacity>
                    </View>
                  </Card>
                  <Text style={styles.repliesHeader}>{replies.length} Replies</Text>
                </MotiView>
              ) : null
            }
            ListEmptyComponent={<View style={styles.empty}><HelpCircle size={48} color={Colors.border} /><Text style={styles.emptyText}>No replies yet. Be the first to help!</Text></View>}
          />
        )}
        ...
        <View style={styles.inputSection}>
          {replyTo && (
            <View style={styles.replyingToBar}>
              <Text style={styles.replyingToText} numberOfLines={1}>Replying to <Text style={{ fontWeight: 'bold' }}>{replyTo.userId?.name}</Text></Text>
          ...
          <View style={styles.inputRow}>
            <TextInput
              ...
              multiline
            />
            <TouchableOpacity onPress={handleSend} disabled={!replyText.trim() || sending} style={[styles.sendBtn, (!replyText.trim() || sending) && styles.sendDisabled]}>
...
const styles = StyleSheet.create({
  ...
  doubtCard: { marginBottom: Spacing.xl, padding: Spacing.md, gap: 12, ...Shadows.card as any },
  doubtHeaderRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  solvedBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: Colors.success + '15', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12 },
  solvedText: { fontSize: FontSize.xs, color: Colors.success, fontWeight: FontWeight.bold },
  doubtTitle: { fontSize: FontSize.xl, fontWeight: FontWeight.bold, color: Colors.textPrimary, lineHeight: 28 },
  doubtDesc: { fontSize: FontSize.base, color: Colors.textSecondary, lineHeight: 24 },
  doubtFooter: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 8, paddingTop: 12, borderTopWidth: 1, borderTopColor: Colors.borderLight },
  doubtMeta: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  doubtUser: { fontSize: FontSize.base, fontWeight: FontWeight.bold, color: Colors.textPrimary },
  doubtTime: { fontSize: FontSize.xs, color: Colors.textMuted },
  doubtUpvote: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: Colors.primary + '10', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20 },
  doubtUpvoted: { backgroundColor: Colors.primary },
  doubtUpvoteText: { fontSize: FontSize.base, fontWeight: FontWeight.bold, color: Colors.primary },
  repliesHeader: { fontSize: FontSize.lg, fontWeight: FontWeight.bold, color: Colors.textPrimary, marginBottom: Spacing.md },
  replyCardContainer: { marginBottom: Spacing.sm, position: 'relative' },
  threadLine: { position: 'absolute', left: -10, top: 0, bottom: 0, width: 2, backgroundColor: Colors.border, borderRadius: 1 },
  replyCard: { padding: Spacing.md, gap: 10, ...Shadows.card as any },
  acceptedCard: { borderWidth: 2, borderColor: Colors.success, backgroundColor: Colors.success + '05' },
  acceptedBadge: { 
    flexDirection: 'row', 
    alignItems: 'center', 
    gap: 4, 
  ...
  acceptedText: { fontSize: FontSize.xs, color: Colors.success, fontWeight: FontWeight.bold },
  replyHeader: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  replyUser: { fontSize: FontSize.base, fontWeight: FontWeight.bold, color: Colors.textPrimary },
  replyTime: { fontSize: FontSize.xs, color: Colors.textMuted },
  acceptBtn: { padding: 4, backgroundColor: Colors.success + '15', borderRadius: 8 },
  replyText: { fontSize: FontSize.base, color: Colors.textSecondary, lineHeight: 22 },
  replyActions: { flexDirection: 'row', alignItems: 'center', gap: 20, marginTop: 4 },
  upvoteRow: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  upvoteCount: { fontSize: FontSize.sm, fontWeight: FontWeight.bold, color: Colors.textMuted },
  replyActionBtn: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  replyActionText: { fontSize: FontSize.sm, fontWeight: FontWeight.medium, color: Colors.textMuted },
  ...
  inputSection: { backgroundColor: Colors.card, borderTopWidth: 1, borderTopColor: Colors.border, paddingBottom: Platform.OS === 'ios' ? 20 : 0 },
  replyingToBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: Spacing.screenPadding, paddingVertical: 8, backgroundColor: Colors.backgroundAlt },
  replyingToText: { fontSize: FontSize.xs, color: Colors.textSecondary },
  inputRow: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: Spacing.screenPadding, paddingVertical: Spacing.md, gap: 10 },
  input: { flex: 1, backgroundColor: Colors.backgroundAlt, borderRadius: BorderRadius.xl, paddingHorizontal: Spacing.md, paddingVertical: 12, fontSize: FontSize.base, color: Colors.textPrimary, maxHeight: 120 },
  sendBtn: { width: 48, height: 48, borderRadius: 24, backgroundColor: Colors.primary, alignItems: 'center', justifyContent: 'center', ...Shadows.md as any },
